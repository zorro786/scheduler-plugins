...
func BenchmarkPerfScheduling(b *testing.B) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
		bytes, err := json.Marshal(watcherMetrics)
		if err != nil {
...
var watcherMetrics watcher.WatcherMetrics
...
func perfScheduling(test testCase, b *testing.B) []DataItem {
	...
	perfTestNodePreparer := nodePreparer.(*SchedulerPerfTestNodePreparer)
	var nodeList *v1.NodeList
	if nodeList, err = perfTestNodePreparer.PrepareNodesV2(); err != nil {
		b.Fatal(err)
	}
	...
	watcherMetrics = watcher.WatcherMetrics{
...
func getNodePreparer(nc nodeCase, clientset clientset.Interface) (testutils.TestNodePreparer, error) {
	...
	if nc.NodeTemplatePath != nil {
		...
		return NewSchedulerPerfTestNodePreparerWithNodeSpec(
			clientset,
			[]testutils.CountToStrategy{{Count: nc.Num, Strategy: nodeStrategy}},
			node,
		), nil
	}
	return NewSchedulerPerfTestNodePreparer(
		clientset,
		[]testutils.CountToStrategy{{Count: nc.Num, Strategy: nodeStrategy}},
		nodeNamePrefix,
	), nil
